...
class MultiscaleBatchSampler(object):
    r"""Samples elements with different scales per batch i.e. when scale == 2 then the dataloader
    __getitem__ will be a tuple with index and scale
    ...
    def __init__(self, sampler: Callable,
        ...
        if multiscale_step is not None and multiscale_step < 1:
            raise ValueError("multiscale_step should be > 0, but got multiscale_step={}".format(multiscale_step))

        if not isinstance(scales, int):
            raise ValueError("scales must a int, got {}".format(scales))
...
class IterationBasedMultiscaleBatchSampler(MultiscaleBatchSampler):
    def __init__(self, batch_sampler: Callable,
                 num_iterations: int,
                 start_iter: int = 0) -> None:
        self.batch_sampler = batch_sampler
        self.num_iterations = num_iterations
        self.start_iter = start_iter
        super(IterationBasedMultiscaleBatchSampler, self).__init__(sampler=batch_sampler.sampler,
