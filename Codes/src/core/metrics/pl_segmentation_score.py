...
class pl_IOU(Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        ...
        area_intersection = torch.histc(intersection, bins=self.num_classes, min=0, max=self.num_classes - 1)
...
def intersectionAndUnionGPU(output: torch.Tensor,
                            target: torch.Tensor,
                            K: int,
                            ignore_index: int = -1,
                            test: bool = False,
...
def intersectionAndUnion(output: np.ndarray,
                         target: np.ndarray,
                         K: int,
