...
class checkpoint():
    def __init__(self, args: argparse.Namespace, test=False):
        self.args = args
        if test:

            self.args.save_dir = os.path.join(self.args.save_dir, 'Evaluation')

        self.paths = {'save_dir': self.args.save_dir,
                      'logs': os.path.join(self.args.save_dir, 'logs')}
        if not test:
            self.paths['ckpt'] = os.path.join(self.args.save_dir, 'ckpt')
    ...
    def get_path(self, subdir: str) -> str:
        if self.paths.__contains__(subdir):
            return self.paths[subdir]
        else:
            temp = os.path.join(self.args.save_dir, subdir)
