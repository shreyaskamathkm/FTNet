...
def pad_to_crop_sz(
    ...
    if pad_h > 0 or pad_w > 0:
        temp = []
        for c in range(ch):
            temp.append(torch.nn.functional.pad(input=image[:, c, :, :],
                                                pad=(pad_w_half, pad_w - pad_w_half, pad_h_half, pad_h - pad_h_half),
                                                mode='constant',
                                                value=mean[c]))  # (padding_left, padding_right, \text{padding\_top}, \text{padding\_bottom})padding_top, padding_bottom)
