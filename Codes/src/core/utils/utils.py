...
__all__ = ['get_rank', 'save_model_summary', 'plot_tensors', 'as_numpy',
           'plot_tensors', 'save_checkpoint', 'print_network',
           'total_gradient', ]
...
def save_model_summary(model, dir_):
    ...
    confg += '\nTotal number of parameters in M: {}M'.format(num_params / (1000**2))
    with open(path, "w") as text_file:
        text_file.write(confg)
...
def print_network(net):
    ...
    for param in net.parameters():
        num_params += param.numel()
    print(net)
    print('Total number of parameters: {} M'.format(int(num_params) / 1000**2))
