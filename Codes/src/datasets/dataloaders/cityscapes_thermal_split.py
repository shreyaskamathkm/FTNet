...
class CityscapesThermalsSplitDataset(SegmentationDataset):
    ...
    def __init__(self, root='./datasets/Cityscapes',
        ...
        super(CityscapesThermalsSplitDataset, self).__init__(root, split, mode, base_size, crop_size, logger)
        self.sobel_edges = sobel_edges
        assert os.path.exists(self.root), "Error: data root path is wrong!"

        self.images, self.mask_paths, self.edge_paths = _get_city_pairs(self.root, self.split, logger)
        ...
        for root, _, files in os.walk(img_folder):
            for filename in files:
                if filename.endswith('.jpg'):
                    ...
                    maskname = filename.replace('leftImg8bit_synthesized_image.jpg', 'gtFine_labelIds.png')
                    maskpath = os.path.join(mask_folder, foldername, maskname)

                    edgename = filename.replace('leftImg8bit_synthesized_image.jpg', 'gtFine_edge.png')
