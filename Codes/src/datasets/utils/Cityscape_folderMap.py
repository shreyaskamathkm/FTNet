...
base_dir = os.path.join(os.path.abspath(os.path.join(basepath, '..')), 'CITYSCAPE_5000')
if reset == True:
...
tile_df = tile_df.sort_values(by='Image_Name', axis=0, ascending=True, kind='quicksort').reset_index(drop=True)
tile_df = tile_df.fillna('NA')
tile_df['Mask_Path'] = tile_df['Image_Path'].str.replace('.jpg', '.png')
tile_df['Mask_Path'] = tile_df['Mask_Path'].str.replace('TIR_leftImg8bit', 'TIR_leftImg8bit/gtFine')
tile_df['Mask_Path'] = tile_df['Mask_Path'].str.replace('leftImg8bit_synthesized_image', 'gtFine_labelIds')
tile_df['Mask_Name'] = tile_df['Image_Name'].str.replace('leftImg8bit_synthesized_image', 'gtFine_labelIds')
...
tile_df = tile_df[~tile_df.Image_Name.str.contains("\(")]  # there are copies of some files, eg, ABCD.png and ABCD (1).png (I am removing the copies)
...
copying(tiles=tile_df, path_label='Image_Path', basepath=base_dir, fileset_path=main_dirs_image[0])
copying(tiles=tile_df, path_label='Mask_Path', basepath=base_dir, fileset_path=main_dirs_mask[0])
