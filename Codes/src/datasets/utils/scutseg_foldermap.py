...
train_Name_path = basepath + 'dataset/scut_train.txt'
val_Name_path = basepath + 'dataset/scut_val.txt'
Image_path = basepath + 'dataset/image/'
Mask_path = basepath + 'dataset/gt_instance/'

# os.path.abspath(os.path.join(basepath,'..'))
base_dir = os.path.join(basepath, 'SCUTSEG')
if reset == True:
...
maskid_path_dict = {os.path.splitext(os.path.basename(x))[0]: x
                    for x in glob(os.path.join(Mask_path, '**', '**', '*.jpg'), recursive=True)}


train_df = pd.read_table(train_Name_path, delim_whitespace=True, names=('Image_subpath', 'Mask_subpath'))
train_df = train_df.sort_values(by='Image_subpath', axis=0, ascending=True, kind='quicksort').reset_index(drop=True)
train_df = train_df.fillna('NA')

train_df['Image_Name'] = train_df['Image_subpath'].apply(lambda x: os.path.splitext(os.path.basename(x))[0])
...
train_df['Mask_Path'] = train_df['Mask_Path'].apply(lambda x: x.split('.')[0] + '_labelIds.png')
...
copying(tiles=train_df1, path_label='Mask_Path', basepath=base_dir, fileset_path=main_dirs_mask[0])


validation_df = pd.read_table(val_Name_path, delim_whitespace=True, names=('Image_subpath', 'Mask_subpath'))
validation_df = validation_df.sort_values(by='Image_subpath', axis=0, ascending=True, kind='quicksort').reset_index(drop=True)
validation_df = validation_df.fillna('NA')

validation_df['Image_Name'] = validation_df['Image_subpath'].apply(lambda x: os.path.splitext(os.path.basename(x))[0])
...
validation_df['Mask_Path'] = validation_df['Mask_Path'].apply(lambda x: x.split('.')[0] + '_labelIds.png')
...
copying(tiles=validation_df1, path_label='Mask_Path', basepath=base_dir, fileset_path=main_dirs_mask[1])
