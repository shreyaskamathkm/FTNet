...
class CyclicLRWithRestarts(_LRScheduler):
    ...
    def get_lr(self, t_cur):
        ...
        weight_decay_norm_multi = math.sqrt(
            self.batch_size / (self.steps_per_epoch * self.restart_period)
        )
        ...
        if self.t_epoch % self.restart_period < self.t_epoch:
            if self.verbose:
                print(f"Restart {self.restarts + 1} at epoch {self.last_epoch}")
    ...
    def _set_batch_increment(self):
        d, r = divmod(self.steps_per_epoch, self.batch_size)
